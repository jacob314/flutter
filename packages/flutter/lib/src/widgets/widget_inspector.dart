...
import 'dart:ui' as ui show window, ClipOp, Image, ImageByteFormat, Paragraph, Picture, PictureRecorder, PointMode, SceneBuilder, Vertices;
...
/// A [Canvas] that multicasts all method calls to a main canvas and a secondary
/// screenshot canvas so that a screenshot can be recorded at the same time
/// as performing a normal paint.
...
/// Data shared between all nested [_ScreenshotPaintingContext] objects painting
/// a screenshot.
class _ScreenshotData {
  ...
  /// Layer tree containing the screenshot.
  ...
  /// Whether paint operations shouldr record to the main regular paint.
  ///
  /// This should only be set to true before paint operations that are not
  /// part of the regular paint such rendering debug information about the
  /// [target].
  bool includeInRegularPaint = true;

  /// Offset of the screenshot equaling the offset [target] was given as
...
class _ScreenshotPaintingContext extends PaintingContext {
  ...
  @override Canvas get canvas {
    if (_data.includeInScreenshot) {
      if (_screenshotCanvas == null) {
        startRecording();
      }
      assert(_screenshotCanvas != null);
      return _data.includeInRegularPaint ? _multicastCanvas : _screenshotCanvas;
    } else {
      assert(_data.includeInRegularPaint);
      return super.canvas;
    }
  }
  ...
  @override
  void startRecording() {
    if (_data.includeInRegularPaint) {
      super.startRecording();
    }
    assert(_data.includeInRegularPaint || _data.includeInScreenshot);
    if (_data.includeInScreenshot) {
  ...
  void _stopRecordingScreenshotIfNeeded() {
    if (!_isScreenshotRecording)
      return;
    // No need to draw repaint rainbows
  ...
  static Future<ui.Image> toImage(
    ...
    if (debugPaint && !debugPaintSizeEnabled) {
      data.includeInRegularPaint = false;
